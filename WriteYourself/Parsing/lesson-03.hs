...
> ./lesson-03.sh
> ./lesson-03 "(a (dotted . list) test)"
> ./lesson-03 "(a '(quoted (dotted . list)) test)"
...
{-
Next, we add a few more parser actions to our interpreter.
Start with the parenthesized lists that make Lisp famous:
-}
parseList :: Parser LispVal
parseList = liftM List $ sepBy parseExpr blanks

{-
The expression char '.' >> spaces returns a Parser (), then combining that
with parseExpr gives a Parser LispVal, exactly the type we 
need for the do-block.
-}
